...
export class UserCityRequiredException extends DomainException {
   constructor() {
      super(UserPasswordRequiredException.getMessage())
      this.name = DomainExceptionCode.USER_CITY_REQUIRED
   }
...
export class UserProvinceRequiredException extends DomainException {
   constructor() {
      super(UserPasswordRequiredException.getMessage())
      this.name = DomainExceptionCode.USER_PROVINCE_REQUIRED
   }
...
export class UserPasswordLengthInvalidException extends DomainException {
   ...
   static getMessage(password: string) {
      return `Password must be more than 6 characters, but '${password}' has only ${password.length}`
   }
}
